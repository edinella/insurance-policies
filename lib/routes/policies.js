...
const textLike = require('../textLike');
...
router.get('/', auth, allowed, async (req, res, next) => {
  ...
  // check filters
  const { clientName, clientId, clientEmail, clientNameLike } = req.query;
  ...
  if (clientNameLike !== undefined) {
    clientConditions.name = textLike(clientNameLike);
  }
